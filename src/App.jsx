...
function statusInRecord(rec, statusKey) {
  const check = (val) => !!val && val.toString().trim().length > 0 && val.toString().toLowerCase().includes(statusKey);
  if (check(rec[statusKey])) return true;
  if (rec.sub_files && rec.sub_files.length) {
    return rec.sub_files.some((s) => check(s[statusKey]));
  }
...
function App() {
  ...
  const results = useMemo(() => {
    ...
    const filtered = records.filter((rec) => {
      if (!recordMatchesQuery(rec, q)) return false;
      if (activeStatusFilters.length > 0 && !activeStatusFilters.some((sf) => statusInRecord(rec, sf))) return false;
      if (!remarkMatches(rec, remarkFilter)) return false;
      return true;
    });
  ...
  return (
    <div className="app-container">
      <header className="app-header">
        <h1>FileSearch</h1>
        <p>Architectural File Management System</p>
      </header>
      <main className="app-main">
        ...
        />
        <FileTable results={results} loading={loading} error={error} />
      </main>
      <footer className="app-footer">
        <p>Created for Saakaar Architects.</p>
      </footer>
    </div>
  );
}
